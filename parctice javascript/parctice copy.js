...
looper(family); // alert => Tyler => Jordyn => Ryan => Chelsey => Ireland
...
reversedLooper(letters); // alert => E => D => C => B => A
...
evenFinder(nums); // return => => [ 2, 6, 22, 98, 22, 12 ]
...
function divider(numbersArray) {
  ...
  console.log(evens); // return => [ 2, 34, 54, 34, 32, 54, 66 ]
  console.log(odds); // return => [ 1, 55, 11, 19, 17, 13 ]
  return [evens, odds];
}
divider(numbersArray); // invoke the function
...
removeItem(myGroceryList, 'chips'); // return => [ 'pizza', 'hotpockets', 'MtnDew', 'corndogs' ]
...
addItem(myGroceryList, "milk"); // return => [ 'chips', 'pizza', 'hotpockets', 'MtnDew', 'corndogs', 'milk' ]
